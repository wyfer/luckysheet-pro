...
export function setBothFrozen(isRange, options = {}) {
	...
	if (!isRange) {
		...
		let isStringRange =
			typeof range === "string" && formula.iscelldata(range);
		if (isCurrentSheet) {
			if (
				(!range ||
					!(
...
export function getRangeValue(options = {}) {
	...
	if (!range || typeof range === "object") {
		...
		if (formula.iscelldata(range)) {
			...
			tooltip.info(
				"The range is invalid, please check range parameter.",
				""
			);
		}
	}
		...
		for (let j = 0; j < colIndexArr.length; j++) {
			...
			if (d[r] != null && d[r][c] != null) {
				...
				if (c == colIndexArr[0]) {
					if (
						cfg["rowlen"] == null ||
						cfg["rowlen"][r.toString()] == null
					) {
						style += "height:19px;";
					} else {
						style +=
							"height:" + cfg["rowlen"][r.toString()] + "px;";
					}
				}
				...
				if (getObjType(d[r][c]) == "object" && "mc" in d[r][c]) {
					...
					if (borderInfoCompute && borderInfoCompute[r + "_" + c]) {
						...
						if (borderInfoCompute[r + "_" + c].l) {
							let linetype =
								borderInfoCompute[r + "_" + c].l.style;
						...
						if (borderInfoCompute[r + "_" + c].r) {
							let linetype =
								borderInfoCompute[r + "_" + c].r.style;
						...
						if (borderInfoCompute[r + "_" + c].b) {
							let linetype =
								borderInfoCompute[r + "_" + c].b.style;
						...
						if (borderInfoCompute[r + "_" + c].t) {
							let linetype =
								borderInfoCompute[r + "_" + c].t.style;
			...
			for (let r = 1; r < getdata.length; r++) {
				...
				for (let c = 0; c < getdata[0].length; c++) {
					if (getcellvalue(0, c, getdata) == undefined) {
						...
						obj[getcellvalue(0, c, getdata)] = getcellvalue(
							r,
							c,
							getdata
						);
...
export function setRangeValue(data, options = {}) {
	...
	if (range instanceof Array) {
		return tooltip.info(
			"setRangeValue only supports a single selection.",
			""
		);
	}
...
export function setRangeConditionalFormatDefault(
	...
	if (conditionName == "betweenness") {
		...
		if (rangeArr1.length > 1) {
			...
			if (r1 == r2 && c1 == c2) {
				...
				conditionformat.infoDialog(
					conditionformat_Text.onlySingleCell,
					""
				);
				return;
			}
		...
		if (diff(v1, v2) > 0) {
			v =
				dayjs(v2).format("YYYY/MM/DD") +
				"-" +
				dayjs(v1).format("YYYY/MM/DD");
		} else {
			v =
				dayjs(v1).format("YYYY/MM/DD") +
				"-" +
		...
		if (parseInt(v) != v || parseInt(v) < 1 || parseInt(v) > 1000) {
			conditionformat.infoDialog(
				conditionformat_Text.pleaseEnterInteger,
				""
			);
			return;
		}
...
/**
 * 删除指定index的工作表，返回已删除的工作表对象
 * @param {Object} options 可选参数
 * @param {String} options.index 工作表下标；默认值为当前工作表下标
 * @param {Function} options.success 操作结束的回调函数
 */
export function deleteSheetByIndex(options = {}) {
	let { index, success } = { ...options };

	if (!index) {
		index = Store.luckysheetfile.find(
			(i) => i.order === Store.currentSheetIndex
		);
	}

	const order = Store.luckysheetfile.findIndex((i) => i.index == index);
	if (order == -1) {
		return tooltip.info("The index parameter is invalid.", "");
	}
	...
	// 通过 index 找在 sheets 中的下标 删除是最准确的
	sheetmanage.deleteSheet(file.index);
	...
	if (isRealNum(targetOrder)) {
		afterObj = $(
			"#luckysheet-sheets-item" +
				Store.luckysheetfile[targetOrder - 1].index
		);
	}
	...
	if (scrollLeft != null) {
		...
		let col = Store.visibledatacolumn[targetColumn],
			col_pre =
				targetColumn <= 0
					? 0
					: Store.visibledatacolumn[targetColumn - 1];
...
export function setDataVerification(optionItem, options = {}) {
	...
	if (type == "dropdown") {
		...
		if (!type2Values_1.includes(type2)) {
			return tooltip.info(
				"The optionItem.type2 parameter is invalid.",
				""
			);
		}
	...
	if (getObjType(hintShow) != "boolean") {
		return tooltip.info(
			"The optionItem.hintShow parameter is invalid.",
			""
		);
	}
	...
	if (file.index == Store.currentSheetIndex) {
		...
		if (visibledatarow.length === 0) {
			...
			for (let r = 0; r < rowheight; r++) {
				...
				if (
					config["rowhidden"] != null &&
					config["rowhidden"][r] != null
				) {
		...
		if (visibledatacolumn.length === 0) {
			...
			for (let c = 0; c < colwidth; c++) {
				...
				if (
					config["columnlen"] != null &&
					config["columnlen"][c] != null
				) {
					firstcolumnlen = config["columnlen"][c];
				}

				if (
					config["colhidden"] != null &&
					config["colhidden"][c] != null
				) {
...
export function getRangeByTxt(txt) {
	...
	if (txt == null) {
		return {
			column: Store.luckysheet_select_save[
				Store.luckysheet_select_save.length - 1
			].column,
			row: Store.luckysheet_select_save[
				Store.luckysheet_select_save.length - 1
			].row,
		};
	}
 ...
 */
export function createChart(sheetIndex, row, column, width, height, top, left) {
	// 生成chartID
	...
	// 调用 controll
	return chartController.createChart(
		sheetIndex,
		chart_id,
		chartData,
		[{ row, column }],
...
/**
 * 更新统计图 - 需要传入生成统计图的行列数据
 * @param {String}  chart_id            统计图ID
 * @param {Object}  options             更新的配置项：宽高 位置 数据等
 */
export function updateChart(data) {
	return chartController.updateChart(data);
}

/**
 * 更新统计图 - 需要传入生成统计图的行列数据
 * @param {String}  sheetIndex          sheet id
 * @param {String}  chart_id            统计图ID
 * @param {Object}  chartOptions        传入的统计图配置项
 * @param {number}  width               图表宽度
 * @param {number}  height              图表高度
 * @param {number}  top                 图表距离顶部距离
 * @param {number}  left                图表距离左侧距离
...
export function renderChart(data) {
	return chartController.renderChart(data);
}
