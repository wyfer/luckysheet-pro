...
export default function luckysheetHandler() {
	...
	$("#luckysheet-sheet-container-c").mousewheel(function (event, delta) {
		let scrollNum =
			event.deltaFactor < 40 ? 1 : event.deltaFactor < 80 ? 2 : 3;
	...
	$("#luckysheet-cell-main, #luckysheetTableContent")
		.mousedown(function (event) {
			...
			if (event.which == "3") {
				...
				for (let s = 0; s < Store.luckysheet_select_save.length; s++) {
					if (
						...
						row_index >=
							Store.luckysheet_select_save[s]["row"][0] &&
						row_index <=
							Store.luckysheet_select_save[s]["row"][1] &&
						col_index >=
							Store.luckysheet_select_save[s]["column"][0] &&
						col_index <=
							Store.luckysheet_select_save[s]["column"][1]
					) {
						isright = true;
						break;
					}
			...
			if (
				...
				if (
					...
					getObjType(luckysheetConfigsetting.fireMousedown) ==
						"function"
				) {
			...
			if (parseInt($input.css("top")) > 0) {
				if (
					...
					setTimeout(function () {
						...
						let $editor;
						if (
							$("#luckysheet-search-formula-parm").is(
								":visible"
							) ||
							$("#luckysheet-search-formula-parm-select").is(
								":visible"
							)
						) {
			...
			if (
				$("#luckysheet-ifFormulaGenerator-singleRange-dialog").is(
					":visible"
				)
			) {
				//选择单个单元格
			...
			if (
				$("#luckysheet-ifFormulaGenerator-multiRange-dialog").is(
					":visible"
				)
			) {
				//选择范围
			...
			if (Store.luckysheet_select_status) {
				if (event.shiftKey) {
					...
					//交替颜色选择范围
					if (
						$("#luckysheet-alternateformat-rangeDialog").is(
							":visible"
						)
					) {
			...
			$(
				"#luckysheet-row-count-show, #luckysheet-column-count-show"
			).hide();
		...
		.mouseup(function (event) {
			if (event.which == "3") {
				...
				$(
					"#luckysheet-cols-rows-add, #luckysheet-cols-rows-shift"
				).hide();
				...
				if (
					...
					for (
						let i = 0;
						i < Store.luckysheet_select_save.length;
						i++
					) {
		...
		.dblclick(function (event) {
			...
			if (pivotTable.isPivotRange(row_index, col_index)) {
				//数据透视表没有 任何数据
				if (
					(pivotTable.filter == null ||
						pivotTable.filter.length == 0) &&
					(pivotTable.row == null || pivotTable.row.length == 0) &&
					(pivotTable.column == null ||
						pivotTable.column.length == 0) &&
				...
				//数据透视表没有 数值数据
				if (
					pivotTable.values == null ||
					pivotTable.values.length == 0
				) {
					return;
				}
				...
				if (pivotTable.row != null && pivotTable.row.length > 0) {
					if (
						pivotTable.values.length >= 2 &&
						pivotTable.showType == "row"
					) {
						if (
							col_index <= pivotTable.row.length ||
							row_index >=
								pivotTable.pivotDatas.length -
									pivotTable.values.length
						) {
							return;
						}
			...
			if (
				...
				$("#luckysheet-modal-dialog-slider-alternateformat").is(
					":visible"
				) ||
	...
	$(document).on("mousemove.luckysheetEvent", function (event) {
		...
		if (formula.functionResizeStatus) {
			...
			$("#luckysheet-wa-calculate").css(
				"height",
				Store.calculatebarHeight - 2
			);
			...
			if (
				Math.abs(event.pageX - Store.luckysheet_sheet_move_data.pageX) <
				3
			) {
				return;
			}
			...
			Store.luckysheet_sheet_move_data.cursorobject.css({
				cursor: "move",
			});
			...
			if (row_index != Store.luckysheet_sheet_move_data.curindex) {
				if (row_index == -1 && left > 0) {
					row_index =
						Store.luckysheet_sheet_move_data.widthlist.length - 1;
			...
			function mouseRender() {
				...
				if (Store.luckysheet_select_status) {
					...
					$("#luckysheet-dataVerificationRange-dialog input").val(
						range
					);
					...
					let row_index_original =
							Store.luckysheet_cell_selected_move_index[0],
						col_index_original =
							Store.luckysheet_cell_selected_move_index[1];
					...
					if (
						col_e >=
							Store.visibledatacolumn[
								Store.visibledatacolumn.length - 1
							] ||
						x > winW
					) {
						col_s =
					...
					col_pre =
						col_s - 1 == -1
							? 0
							: Store.visibledatacolumn[col_s - 1];
					col = Store.visibledatacolumn[col_e];
					row_pre =
						row_s - 1 == -1 ? 0 : Store.visibledatarow[row_s - 1];
					...
					let mouse = mouseposition(event.pageX, event.pageY);
					let scrollLeft =
						$("#luckysheet-cell-main").scrollLeft() - 5;
					...
					let row_index_original =
						...
						col_index_original =
							Store.luckysheet_cell_selected_extend_index[1];
					...
					if (
						...
						if (!(row_index >= row_s && row_index <= row_e)) {
							if (
								...
								height =
									Store.luckysheet_select_save[0].top_move +
									Store.luckysheet_select_save[0]
										.height_move -
									row_pre;
							} else {
					...
					if (
						x + 3 - Store.luckysheet_cols_change_size_start[0] >
							30 &&
					...
					if (
						y + 3 - Store.luckysheet_rows_change_size_start[0] >
							19 &&
				...
				// chart move
					...
					const x =
						mouse[0] + $("#luckysheet-cell-main").scrollLeft();
					const y = mouse[1] + $("#luckysheet-cell-main").scrollTop();

					const myh =
							Store.chartparam.luckysheetCurrentChartMoveObj.height(),
						myw =
							Store.chartparam.luckysheetCurrentChartMoveObj.width();
					let top =
							y -
							Store.chartparam.luckysheetCurrentChartMoveXy[1],
						left =
							x -
							Store.chartparam.luckysheetCurrentChartMoveXy[0];
					...
					if (
						...
						const toffset =
							Store.chartparam.luckysheetCurrentChartMoveObj.offset();
						const tpsition =
							Store.chartparam.luckysheetCurrentChartMoveObj.position();
				...
				// chart resize
					...
					const myh =
						...
						myw =
							Store.chartparam.luckysheetCurrentChartResizeObj.width();
					const topchange =
							y -
					...
					Store.chartparam.luckysheetCurrentChartResizeObj.css(
						resizedata
					);
					// resize chart
				...
				//image resize
					...
					if (imgItem.isFixedPos) {
						...
						if (resize == "lt") {
							...
							if (
								left >
								resizeXY[4] - resizeXY[6] + resizeXY[2] - 1
							) {
								left =
									resizeXY[4] - resizeXY[6] + resizeXY[2] - 1;
							}
							...
							width =
								resizeXY[4] - resizeXY[6] + resizeXY[2] - left;

							height = Math.round(
								width * (resizeXY[3] / resizeXY[2])
							);
							top =
								resizeXY[5] -
								resizeXY[7] +
							...
							if (
								top >
								resizeXY[5] - resizeXY[7] + resizeXY[3] - 1
							) {
								top =
									resizeXY[5] - resizeXY[7] + resizeXY[3] - 1;
								height =
							...
							height =
								resizeXY[5] - resizeXY[7] + resizeXY[3] - top;
						} else if (resize == "mb") {
							//中下
							left = resizeXY[4] - resizeXY[6];
							width = resizeXY[2];
					...
					let defaultWidth = Math.round(
						imgItem.default.width * scaleX
					);
					let defaultHeight = Math.round(
						imgItem.default.height * scaleY
					);
					let offsetLeft = Math.round(
					...
					$(
						...
						"background-size":
							defaultWidth + "px " + defaultHeight + "px",
						"background-position":
							-offsetLeft + "px " + -offsetTop + "px",
					});
				}
				//image cropChange
					...
					if (cropChange == "lt") {
						...
						if (
							offsetLeft >
							imgItem.crop.width + imgItem.crop.offsetLeft - 1
						) {
							offsetLeft =
								imgItem.crop.width +
								imgItem.crop.offsetLeft -
								1;
						}
						...
						if (
							offsetTop >
							imgItem.crop.height + imgItem.crop.offsetTop - 1
						) {
							offsetTop =
								imgItem.crop.height +
					...
					let imageUrlHandle =
						Store.toJsonOptions &&
						Store.toJsonOptions["imageUrlHandle"];
					let imgSrc =
					...
					$(
						...
						"background-image": "url(" + imgSrc + ")",
						"background-size":
							imgItem.default.width +
							"px " +
							imgItem.default.height +
							"px",
						"background-position":
							-offsetLeft + "px " + -offsetTop + "px",
					});
					...
					if (resize == "lm" || resize == "lt" || resize == "lb") {
						...
						if (left > resizeXY[2] + resizeXY[4] - 60) {
							...
							width =
								resizeXY[2] -
								(resizeXY[2] + resizeXY[4] - 60 - resizeXY[0]);
						} else if (left <= 0) {
							left = 0;
							width = resizeXY[2] + resizeXY[0];
						}
					...
					if (resize == "mt" || resize == "lt" || resize == "rt") {
						...
						if (top > resizeXY[3] + resizeXY[5] - 60) {
							...
							height =
								resizeXY[3] -
								(resizeXY[3] + resizeXY[5] - 60 - resizeXY[1]);
						} else if (top <= 0) {
							top = 0;
							height = resizeXY[3] + resizeXY[1];
						}
				...
				Store.jfautoscrollTimeout =
					window.requestAnimationFrame(mouseRender);
			}
	...
	$(document).on("mouseup.luckysheetEvent", function (event) {
		...
		if (!!luckysheetFreezen.horizontalmovestate) {
			...
			if (
				luckysheetFreezen.freezenhorizontaldata[4] <=
				Store.columnHeaderHeight
			) {
		...
		if (!!luckysheetFreezen.verticalmovestate) {
			...
			$("#luckysheet-freezebar-vertical")
				...
				.css("cursor", "-webkit-grab");
			if (
				luckysheetFreezen.freezenverticaldata[4] <= Store.rowHeaderWidth
			) {
				...
				if ($cur.length == 0) {
					...
					$(
						...
						.each(function () {
							...
							$("#luckysheet-modal-dialog-pivotTable-list")
								.find(
									".luckysheet-modal-dialog-slider-list-item"
								)
								.each(function () {
		...
		if (!!Store.chartparam.luckysheetCurrentChartMove) {
			...
			if (Store.chartparam.luckysheetInsertChartTosheetChange) {
				...
				var scrollLeft1 =
					Store.chartparam.luckysheetCurrentChartMoveXy[4];
				var scrollTop1 =
					Store.chartparam.luckysheetCurrentChartMoveXy[5];
		...
		if (!!Store.chartparam.luckysheetCurrentChartResize) {
			...
			if (Store.chartparam.luckysheetInsertChartTosheetChange) {
				...
				var scrollLeft1 =
					Store.chartparam.luckysheetCurrentChartResizeXy[6];
				var scrollTop1 =
					Store.chartparam.luckysheetCurrentChartResizeXy[7];
		...
		if (Store.luckysheet_rows_change_size) {
			...
			if (y >= winH - 200 + scrollTop) {
				size =
					winH -
					200 -
					Store.luckysheet_rows_change_size_start[0] +
					scrollTop;
			}
				...
				Store["luckysheet_select_save"]
					...
					.forEach((select) => {
						for (let r = select.row[0]; r <= select.row[1]; r++) {
							cfg["rowlen"][r] = Math.ceil(
								size / Store.zoomRatio
							);
						}
					});
			} else {
				cfg["rowlen"][Store.luckysheet_rows_change_size_start[1]] =
			...
			server.saveParam("all", Store.currentSheetIndex, images, {
				k: "images",
			});
		...
		if (Store.luckysheet_cols_change_size) {
			...
			if (x >= winW - 100 + scrollLeft) {
				size =
					winW -
					100 -
					Store.luckysheet_cols_change_size_start[0] +
					scrollLeft;
			}
			...
			if (changeColumnSelected) {
				Store["luckysheet_select_save"]
					...
					.forEach((select) => {
						for (
							let r = select.column[0];
							r <= select.column[1];
							r++
						) {
							cfg["columnlen"][r] = Math.ceil(
								size / Store.zoomRatio
							);
						}
					});
			} else {
				cfg["columnlen"][Store.luckysheet_cols_change_size_start[1]] =
					Math.ceil(size / Store.zoomRatio);
			}
		...
		if (Store.luckysheet_cell_selected_move) {
			...
			if (
				row_e >=
					Store.visibledatarow[Store.visibledatarow.length - 1] ||
				y > winH
			) {
				row_s =
					Store.visibledatarow.length -
	...
	$("#luckysheet-cell-main div.luckysheet-cs-fillhandle")
		.mousedown(function (event) {
			...
			Store.luckysheet_cell_selected_extend_time = setTimeout(
				function () {
					...
					let mouse = mouseposition(event.pageX, event.pageY);
					let x =
						mouse[0] + $("#luckysheet-cell-main").scrollLeft() - 5;
					let y =
						mouse[1] + $("#luckysheet-cell-main").scrollTop() - 5;
		...
		if (
			...
			label: for (
				...
				for (let r = r1; r <= r2; r++) {
					for (let c = c1; c <= c2; c++) {
						if (
							conditionformat.checksCF(r, c, cf_compute) != null
						) {
							hasCF = true;
							continue label;
						}
	...
	$("#luckysheet-chart-btn-screenshot").click(function () {
		...
		if (st_r - 1 < 0) {
			...
			rh_height =
				Store.visibledatarow[ed_r] - Store.visibledatarow[st_r - 1];
		}
		...
		if (st_c - 1 < 0) {
			...
			ch_width =
				Store.visibledatacolumn[ed_c] -
				Store.visibledatacolumn[st_c - 1];
		}
	...
	$("#luckysheet-wa-calculate-size").mousedown(function (e) {
		...
		formula.functionResizeData.calculatebarHeight =
			Store.calculatebarHeight;
	...
	let dpi_x =
		document.getElementById("testdpidiv").offsetWidth *
		Store.devicePixelRatio;
	let dpi_y =
		document.getElementById("testdpidiv").offsetHeight *
		Store.devicePixelRatio;
		...
		if (selection.isPasteAction) {
			...
			if (
				...
				let copy_r1 = Store.luckysheet_copy_save["copyRange"][0].row[0],
					...
					copy_c1 =
						Store.luckysheet_copy_save["copyRange"][0].column[0],
					copy_c2 =
						Store.luckysheet_copy_save["copyRange"][0].column[1];
			...
			if (
				...
				if (Store.luckysheet_paste_iscut) {
					...
					selection.pasteHandlerOfCutPaste(
						Store.luckysheet_copy_save
					);
					selection.clearcopy(e);
				} else {
					selection.pasteHandlerOfCopyPaste(
						Store.luckysheet_copy_save
					);
				}
				...
				if (
					...
					($content.find("table").length !== 0 ||
						$content.children("span[data-sheets-value]").length ===
							1)
				) {
					...
					$content.find("table tr").each(function () {
						...
						$tr.find(cellElements).each(function () {
							...
							const originalFormula = $td.attr(
								"data-sheets-formula"
							);
							const originalText = $td.text();
							if (
								originalFormula &&
								originalFormula.startsWith("=")
							) {
							...
							let fs = Math.round(
								(parseInt($td.css("font-size")) * 72) / 96
							);
							cell.fs = fs;
									...
									for (let cp = 0; cp < colspan; cp++) {
										if (rp == 0) {
											...
											if (
												bt != null &&
												bt.length > 0 &&
												bt.substr(0, 3).toLowerCase() !=
													"0px"
											) {
												let width =
													$td.css("border-top-width");
												let type =
													$td.css("border-top-style");
												let color =
													$td.css("border-top-color");
												let borderconfig =
													menuButton.getQKBorder(
														width,
														type,
														color
													);
			...
			$(e.target).closest("#luckysheet-rich-text-editor").length > 0
		) {
			// 阻止默认粘贴
			e.preventDefault();
				...
				method.addDataAjax(
					...
					function () {
						...
						if (
							...
							let pageInfoFull = replaceHtml(
								locale_info.pageInfoFull,
								{
									total: luckysheetConfigsetting.total,
									totalPage:
										luckysheetConfigsetting.pageInfo
											.totalPage,
								}
							);
							$("#luckysheet-bottom-page-info").html(
								pageInfoFull
							);
						} else {
